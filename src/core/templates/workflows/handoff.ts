 ...
 *
 * /opsx:handoff — compact the current session into a handoff document under
 * docs/handoff/ so a fresh agent can pick up the work.
 *
 * Adapted from Matt Pocock's `handoff` skill (mattpocock/skills). The only
 * substantive change: the document is written into the workspace at
 * docs/handoff/<date>-<slug>.md instead of the OS temporary directory, so the
